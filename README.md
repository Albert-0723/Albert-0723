- 👋 嗨，我是 @Albert-0723

- 👀 我的興趣是探索人工智慧和機器學習的世界，以及如何將它們應用於現實生活問題。

- 🌱 目前正在學習更多有關深度學習模型和自然語言處理的知識。

- 💞️ 我正在尋找合作機會，一起開發創新型應用程式，或者參與有趣的開源專案。

- 📫 您可以發送電子郵件至 Powtron.albert@gmail.com 或在社群媒體上找到我。

- 😄 代名詞：我有自己的看法，但也理解並尊重別人的選擇。

- ⚡ 我開發了一套程式，結合 CCD 與機器手臂，實現待測物的自動取放。透過演算法，機器手臂能判斷測試結果（Pass/NG），並將成品正確送往出料位置。

| Repository Name                    | Description                                                                          |
|:----------------------------------:| ------------------------------------------------------------------------------------ |
| Powtron-ATS-2026                   | 產線自動化測試程式備份                                                                          |
| Albert-0723.github.io              | Albert's Home Page                                                                   |
| InverterSimulator                  | Inverter 模擬、通訊攔截                                                                     |
| The-Architectonic-Codex            | From Code to Strategy, Transforming Engineering Fragments into Systemic Wisdom.      |
| AI-Knowledge                       | ersonal knowledge base managed with Obsidian, designed for RAG and LLM applications. |
| Powtron-ATS-2025                   | 產線自動測試系統程式                                                                           |
| Punch-Data-Upload-Tool             | 104 打卡 API 串接，打卡資料上傳                                                                 |
| Small-board-and-work-order-binding | 小板綁定工單 v1.0.0.34                                                                     |
| Albert-0723                        | Repositories List (Readme.md)                                                        |
| EERW-backup                        | EEPROM 讀寫 (& 匯入/匯出)、Record 讀取 (& 匯出)                                                 |
| Program-en-decryption              | 原始程式加密、解密                                                                            |
| Inverter-Durability-Test           | 耐受性測試                                                                                |
| DSC-Monitoring-Tools               | for DSC-1200A-48                                                                     |
| DSC-Write-Date                     | DSC Write Date                                                                       |
| ASP-Script-Generator               | ASP 調整、成測步驟自動產生器                                                                     |
| Datalog-Importer                   | 測試結果資料匯入                                                                             |
| Datalog-exporter                   | 測試結果資料查詢                                                                             |
| New-Relay-Board-Tools              | 繼電器控制板工具程式                                                                           |
| New-Test-Terminal                  | 測試用終端機                                                                               |
|                                    |                                                                                      |

# 📑 SOP 快速選擇表 (Gemini vs Copilot)

| 使用情境 / Scenario                       | 建議工具 / Recommended Tool | 判斷依據 / Decision Basis           |
| ------------------------------------- | ----------------------- | ------------------------------- |
| 📧 郵件撰寫與回覆 / Email drafting & replies | **Copilot**             | 深度整合 Outlook、Teams，支援企業工作流      |
| 📊 數據分析 / Data analysis               | **Copilot**             | Excel 自動化、Power BI 整合，適合 SOP 報表 |
| 📝 文件標準化 / Document standardization   | **Copilot**             | Word、PowerPoint 模組化套用，利於 SOP 建立 |
| 🌐 跨語言研究 / Cross‑language research    | **Gemini**              | 多語言推理能力強，適合跨文化文件                |
| 🎨 創意生成 / Creative ideation           | **Gemini**              | 擅長長文本推理、跨模態輸入 (文字+圖片)           |
| 📚 教學資源 / Educational resources       | **Gemini**              | 大上下文視窗，適合長篇教材與跨模態教學             |
| 🤝 團隊協作 / Team collaboration          | **Copilot**             | 與 Teams、SharePoint 整合，支援多人協作    |
| 🔍 長文本推理 / Long‑context reasoning     | **Gemini**              | 可處理高達 100 萬 tokens 的內容          |
| ⚙️ 工作流程自動化 / Workflow automation      | **Copilot**             | 支援 Power Automate、企業級流程設計       |

---

## 🛠 SOP 判斷流程 (Decision Flow)

1. **先確認工作環境**  
   
   - 使用 Microsoft 365 → 優先 Copilot  
   - 使用 Google Workspace → 優先 Gemini  

2. **再確認任務性質**  
   
   - 商務、數據、文件 → Copilot  
   - 創意、研究、跨語言 → Gemini  

3. **最後考量上下文需求**  
   
   - 長文本推理 → Gemini  
   - 自動化與協作 → Copilot  